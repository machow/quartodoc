...
def get_function(module: str, func_name: str, parser: str = "numpy") -> dc.Object:
	...
	"""
	griffe = GriffeLoader(docstring_parser=Parser(parser), docstring_options=get_parser_defaults(parser))
	mod = griffe.load(module)
...
def _insert_contents(x: dict | list, contents: list, sentinel: str = "{{ contents }}"):
...
class Builder:
	...
	def write_doc_pages(self, pages, filter: str):
		...
		for page in pages:
			...
			if self.rewrite_all_pages or (not html_path.exists()) or (html_path.read_text() != rendered):
	...
	def from_quarto_config(cls, quarto_cfg: "str | dict"):
		...
		return cls_builder(**{k: v for k, v in cfg.items() if k != "style"}, _fast_inventory=_fast_inventory)
